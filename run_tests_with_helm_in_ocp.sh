...
helm install crud ./helm --set app.route.expose=true --set app.s2i.source.repo=$SOURCE_REPOSITORY_URL --set app.s2i.source.ref=$SOURCE_REPOSITORY_REF --set app.s2i.builderImage.repo=$S2I_BUILDER_IMAGE_REPO --set app.s2i.builderImage.tag=$S2I_BUILDER_IMAGE_TAG
