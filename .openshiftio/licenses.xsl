...
<xsl:stylesheet version="1.0"
            ...
            <tr>
              ...
              <td>
                <xsl:for-each select="licenses/license">
                  ...
                  <a href="content/{$filename}"><xsl:value-of select="$filename"/></a><br/>
                </xsl:for-each>
              </td>
            </tr>
          </xsl:for-each>
  ...
  <xsl:template name="remap-local-filename">
    ...
    <xsl:choose>
      ...
      <xsl:when
          test="$filename = 'common development and distribution license (cddl) and gnu public license v.2 w/classpath exception - cddl-gplv2.html'">
        <xsl:value-of
            select="'common development and distribution license (cddl) and gnu public license v.2 w_classpath exception - cddl-gplv2.html'"/>
      </xsl:when>
      <xsl:otherwise>
